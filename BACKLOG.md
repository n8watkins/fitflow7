...
- **Verify (must stay green):** `npx tsc -b && npm run lint && npm run test && npm run build` (+ `cd mcp && npm run typecheck`). **123 tests** today. CI (`.github/workflows/ci.yml`) runs the whole pipeline on push/PR.
