...
- **Verify pipeline:** `npx tsc -b` + `npm run lint` + `npm run test` (**116 tests**) + `npm run build` — all green. `/api` type-checked via `tsconfig.api.json`; `mcp/` has its own `npm run typecheck`. **CI** (`.github/workflows/ci.yml`) now runs the full pipeline on push/PR.
...
- **Cloud sync for body stats (B1):** body profile + weight log + challenge progress now sync (Turso `weight_log`/`body_profile`/`challenge_progress`, user-scoped LWW + tombstones) on the same dirty-queue contract as routines/sessions; read-only tokens can't push them. Plus a self-review pass: light-theme variants for the workout Player's phase tiles, Stats summary grid fix. Tests 77 → **116**.
